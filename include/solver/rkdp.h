...
class rkdp : public adaptive_step_size_solver<dtype, order, linalg_engine> {
 public:
  ...
  const dtype B[6] = { frac<dtype>(35, 384),
                       zero<dtype>(),
                       frac<dtype>(500, 1113),
                       frac<dtype>(125, 192),
  ...
  const dtype E[7] = { frac<dtype>(-71, 57600),
                       zero<dtype>(),
                       frac<dtype>(71, 16695),
                       frac<dtype>(-71, 1920),
                           ...
                           const kwargs_t                        &kwargs)
  {
    ...
    while (not accepted) {
      ...
      // start = std::chrono::system_clock::now();
      real_t<dtype> error_norm = errnrm1<dynamic>(this->engine,
                                                  this->kh[0],
                                                  rho,
                                                  this->rho_old,
                                                  this->atol,
      ...
      if (error_norm < one<real_t<dtype>>()) {
        t       += dt_1;
        dt_1    *= safety * std::min(frac<real_t<dtype>>(10, 1),
                                     std::pow(error_norm, error_exponent));
        accepted = true;
      } else {
