 ...
 public:
  ...
  void set_param(linalg_engine *obj)
  {
    ...
    for (int s = 0; s < this->n_noise; ++s) {
      ...
      kron_x_1<dynamic>(nilobj, +i_unit<dtype>(), this->Lambda[s],     zero<dtype>(),
                        this->Theta[s]);
      kron_1_x_T<dynamic>(nilobj, -i_unit<dtype>(), this->Lambda_dgr[s],  one<dtype>(),
                          this->Theta[s]);
    }
...
#endif // ifndef LIBHEOM_REDFIELD_LIOU_H
