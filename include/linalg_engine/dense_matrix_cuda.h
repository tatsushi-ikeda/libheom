...
class dense_matrix<dynamic, dtype, order, cuda>:
 ...
 public:
  ...
  void set_shape(int rows, int cols)
  {
    ...
    CUDA_CALL(cudaMalloc(&this->data_dev,
                         std::get<0>(this->shape) * std::get<1>(this->shape) * sizeof(dtype)));
    CUDA_CALL(cudaMemset(this->data_dev, 0,
                         std::get<0>(this->shape) * std::get<1>(this->shape) * sizeof(dtype)));
  }
  /* ---------------------------------------------------------------------- */
  ...
  void import(lil_matrix<dynamic, dtype, order, nil> &src)
  {
    ...
    data =
      new (std::align_val_t{ align_val<dtype> }) dtype [std::get<0>(this->shape) *
                                                        std::get<1>(this->shape)];
    ...
    CUDA_CALL(cudaMalloc(&this->data_dev,
                         std::get<0>(this->shape) * std::get<1>(this->shape) * sizeof(dtype)));
    CUDA_CALL(cudaMemcpy(this->data_dev, data,
                         std::get<0>(this->shape) * std::get<1>(this->shape) * sizeof(dtype),
  ...
  void dump(lil_matrix<dynamic, dtype, order, nil> &dest)
  {
    ...
    data =
      new (std::align_val_t{ align_val<dtype> }) dtype [std::get<0>(this->shape) *
                                                        std::get<1>(this->shape)];
    CUDA_CALL(cudaMemcpy(data, this->data_dev,
                         std::get<0>(this->shape) * std::get<1>(this->shape) * sizeof(dtype),
...
#endif // ifndef LIBHEOM_DENSE_MATRIX_CUDA_H
